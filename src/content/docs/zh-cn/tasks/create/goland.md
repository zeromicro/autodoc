---
title: 创建项目（GoLand）
description: 通过 JetBrains GoLand 脚手架创建新的 go-zero 项目。
sidebar:
  order: 3
...
![create from goland](/resource/tasks/create/run-in-goland1.png)
![create from goland](/resource/tasks/create/run-result-in-goland1.png)

1. 在 `main.go` 文件内容面板中，找到第 5 行代码 `func main() {` 左边的三角符号，点击运行即可。

![create from goland](/resource/tasks/create/run-in-goland2.png)
![create from goland](/resource/tasks/create/run-result-in-goland2.png)
...
1. 在 `main.go` 文件上右键，选择 `Open In`，然后选择 `Terminal`，即可打开终端，然后输入如下指令运行即可。
    ...
    ![create from goland](/resource/tasks/create/run-in-goland3.png)
    ![create from goland](/resource/tasks/create/run-result-in-goland3.png)
