---
title: Docker 部署
description: 使用 Docker 构建和部署 go-zero 服务，包括多阶段构建和 GitHub Actions CI/CD 自动化。
sidebar:
  order: 4
...
![ alt='docker-deployment' ](/resource/tutorials/ops/docker-deployment-1.png)
...
7. **Login to Docker Hub**：
   ...
   ![ alt='docker-deployment' ](/resource/tutorials/ops/docker-deployment-2.png)

   记得把 token 复制保存下来，这个界面关闭后就再也看不到token了

   - Github Secret 会作为环境变量在Github Action运行中使用，不会暴露在日志中，因此特别适合存放token或者密码。

   ![ alt='docker-deployment' ](/resource/tutorials/ops/docker-deployment-3.png)
