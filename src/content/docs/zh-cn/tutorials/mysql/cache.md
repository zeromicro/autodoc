---
title: MySQL 缓存
description: 使用 go-zero sqlc 为 MySQL 查询添加缓存。
sidebar:
  order: 4
...
go-zero 除了提供 sqlx.SqlConn, 我们也提供了一个 sqlc.CachedConn 的封装，用于sql 数据库缓存的支持，我们建议如下代码使用 [goctl model -c](./connection) 进行生成，无需手动录入。
简单实例如下：
...
我们可以通过**NewConnWithCache** 方法创建一个 CachedConn，其中 db 就是我们sqlx中的 [SqlConn](./connection)，需要用户自行创建。
