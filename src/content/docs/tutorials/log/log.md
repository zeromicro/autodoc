---
title: Logging
description: Configure and use go-zero's structured logging (logx).
---
...
We provide rich log setup capability, which can be configured by configuration. For details see [log configuration](../configuration/log)
...
go-zero has two ways to set level, one set by configuration, see[log configuration](../configuration/log)
...
go-zero in file output mode supports split between two files by day and by size. For more information see[Rotation](../configuration/log)
