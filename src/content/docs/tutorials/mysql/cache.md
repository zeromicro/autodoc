---
title: MySQL Caching
description: Add caching to MySQL queries with go-zero sqlc.
sidebar:
  order: 4
...
go-zero, in addition to providing sqlx.SqlConn, we also provide a sqlc.CachedConn encapsulation support for sql database cache. We recommend the following code to be generated using [goctl model -c](./connection) without manual entry. Simple Example：
...
We can create a CachedConn with the **NewConnWithCache** method, where db is the [SqlConn](./connection) in our sqlx, which needs to be created by the user.
