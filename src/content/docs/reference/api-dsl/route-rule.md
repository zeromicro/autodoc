...
1. A router node can contain `:`but `:` must be the first character of a router node,`:` The next node value must have `path in the checkout body` tag statement, used to receive routing parameters, refer to detailed rules <a href="/reference/api-dsl/parameter" target="_blank">routing parameters</a>.
1. Route nodes can contain letters, numbers (`goctl 1.5.1` , reference<a href="/reference/api-dsl/faq#1-%E6%80%8E%E4%B9%88%E4%BD%93%E9%AA%8C%E6%96%B0%E7%9A%84-api-%E7%89%B9%E6%80%A7" target="_blank"> new API solver use</a>), underscore, dash
